...
router.get('/pagination/paginationFilter', async (req, res) => {console.log(req)
    const page = req.query.page || 1; // Current page
    const limit = req.query.limit || 5; // Number of items per page
    const searchTerm = req.query.searchTerm || ""; // searchedTerm
    const offset = (page - 1) * limit;
    // retrouver le prix le plus grand : Utilisation de findOne avec un tridescendant sur le champ prix et ne retourne que le champ prix
    //Le champ _id est toujours présent sauf si on l'exclue explicitement. onajoute la syntaxe - à select
    const maxValueDoc = await Article.findOne().sort('-prix').select('prix -_id');
    const prixMax = req.query.prixMax || maxValueDoc.prix; // le prix max sivide prend la valeur max de la BD
    try {
        let query = {};
        // Si le terme de recherche est défini, ajouter un filtre pour la
        ...
        .skip(offset)
        .limit(limit);
        console.log(articlesTot)
        res.status(200).json({ articles, tot: articlesTot ,
        maxValuePrix:maxValueDoc.prix});
        } catch (error) {
        res.status(404).json({ message: error.message });
        }
        });
